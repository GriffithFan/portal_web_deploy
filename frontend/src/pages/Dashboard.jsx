...
const ConnectivityBar = ({ ap, device }) => {
  ...
  // Usar historial de conectividad para switches si existe, si no usar wireless
  const connectivityHistory = Array.isArray(targetDevice.connectivityHistory) ? targetDevice.connectivityHistory : null;
  const wireless = targetDevice.wireless || {};
  const history = connectivityHistory || (Array.isArray(wireless.history) ? wireless.history : []);
...
export default function Dashboard({ onLogout }) {
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 12 }}>
                <span style={{ 
                  ...
                  <svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg" style={{ marginRight: 4 }}>
                    ...
                    <path d="M5.5 8.5L7.5 10.5L10.5 6.5" stroke="white" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
                  </svg>
                ...
                <div key={appliance.device.serial} style={{ border: '1px solid #cbd5e1', borderRadius: 12, padding: '20px', background: '#fff', display: 'flex', flexDirection: 'column', gap: 20 }}>
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 12, paddingBottom: 16, borderBottom: '2px solid #e2e8f0' }}>
                    <div>
                      ...
                      {/* Puerto LAN conectado al switch */}
                      {Array.isArray(appliance.ports) && (
                        (() => {
                          // Buscar puerto LAN conectado y online
                          const lanPort = appliance.ports.find(p => {
                            const isLan = (p.role === 'lan' || p.type === 'lan' || (!p.role && !p.type));
                            const isConnected = normalizeReachability(p.statusNormalized || p.status) === 'connected';
                            return isLan && isConnected;
                          });
                          if (lanPort) {
                            return (
                              <div style={{ marginTop: 6, fontSize: 13, color: '#059669', fontWeight: 600 }}>
                                <span>Puerto LAN conectado: <b style={{ color: '#059669' }}>{lanPort.number || lanPort.portId}</b></span>
                              </div>
                            );
                          }
                          return null;
                      ...
                      {(() => {
                        ...
                        return (
                          <AppliancePortsMatrix
                            ...
                            highlightConnectedColor="#059669"
                          />
                        );
                      })()}
                    </div>
  ...
  return (
    <div style={{ width: '100vw', overflow: 'visible' }}>
      <TopBar onSearch={search} onLogout={onLogout} />
      <div style={{ 
        display: 'grid', 
